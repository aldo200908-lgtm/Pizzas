      ...
      <nav className="container mx-auto px-6 flex justify-between items-center h-full">
        ...
        <div className="hidden lg:flex gap-8 items-center">
          ...
          <div className="flex items-center gap-4 ml-4 pl-4 border-l border-white/10">
            <button 
              onClick={onCartOpen} 
              className="relative p-2.5 bg-white/5 hover:bg-white/10 rounded-full text-white transition-all border border-white/5 group"
            ...
            >
              ...
              {cartCount > 0 && (
                <span className="absolute -top-1 -right-1 bg-[#c0392b] text-white text-[9px] font-black w-5 h-5 flex items-center justify-center rounded-full shadow-lg ring-2 ring-[#0a0a0a] animate-scaleIn">
                  {cartCount}
                </span>
              )}
            </button>
        ...
        <div className="flex items-center gap-3 lg:hidden">
          <button 
            onClick={onCartOpen} 
            className="relative p-3 bg-white/5 rounded-full text-white z-[210] active:scale-95 transition-all"
            aria-label="Abrir carrito"
          >
            ...
            {cartCount > 0 && (
              <span className="absolute -top-0.5 -right-0.5 bg-[#c0392b] text-white text-[9px] font-black w-5 h-5 flex items-center justify-center rounded-full ring-2 ring-[#0a0a0a] animate-scaleIn">
                {cartCount}
              </span>
            )}
          </button>
        ...
        {/* Mobile Menu Overlay */}
        <div className={`fixed inset-0 bg-[#0a0a0a] z-[205] flex flex-col items-center justify-center transition-all duration-500 lg:hidden ${
          ...
          <div className="flex flex-col gap-8 text-center">
            {navLinks.map((link) => (
              <a 
                ...
                href={link.href} 
                onClick={(e) => handleLinkClick(e, link.href)} 
                className="text-2xl font-serif font-bold text-white hover:text-[#facc15] transition-colors"
              >
                {link.name}
              </a>
            ))}
