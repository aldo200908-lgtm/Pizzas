...
const App: React.FC = () => {
  ...
  const addToCart = (product: PizzaItem) => {
    ...
    showToast(`¡${product.name} añadida al pedido!`);
  };
  ...
  return (
    <div className="min-h-screen selection:bg-[#c0392b] selection:text-white animate-fadeIn">
      ...
      <Navbar isScrolled={isScrolled} cartCount={cart.reduce((acc, item) => acc + item.quantity, 0)} onCartOpen={() => setIsCartOpen(true)} />
      <main>
        <Hero />
        <About />
